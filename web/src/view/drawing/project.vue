<template>
	<el-row :gutter="20" style="margin-top: 0;padding-top: 0;">
		...
		<el-col :span="16" class="left">
			<div>
				...
				<el-carousel :interval="10000" type="card" height="570px">
					<el-carousel-item v-for="item, idx in list_template" :key="item.id" class="image">
						<h4 style="text-align: center;">{{ item.title }}</h4>
						<el-image :src="`${path}/` + item.imgSrc" fit="cover" style="width: 100%; height: 570px"
		...
		<div>
			...
			<!-- 新增模板弹窗 -->
			<el-dialog v-model="dialogFormVisible2" title="新增模板">
				<el-form :model="prjectForm" label-width="100px">
					...
					<el-form-item label="模板图片">
						<el-upload :action="`${path}/fileUploadAndDownload/upload?noSave=1`" :before-upload="checkFile"
							:headers="{ 'x-token': userStore.token }" :on-error="uploadError" :on-success="uploadSuccess" :limit="1"
...
<script setup>
import {
	CreateProject, GetAllProject, GetAllBranch, CreateBranch,
	DeleteBranch
...
const openProject = async (item) => {
	//首先在setup中定义
	router.push({ name: 'godrawing', query: { projectid: item.ID }})
	// console.log(item)
}
</script>
...
.el-carousel__item:nth-child(2n) {
	background-color: #babfc7c5;
}

.el-carousel__item:nth-child(2n + 1) {
	background-color: #d3dce6d6;
}
