  ...
  <div>
    ...
    <div class="gva-table-box bottom-mg-lg">
      <div class="gva-btn-list">
        ...
        <el-button type="primary" icon="delete" @click="delSmsProject">删除选中项目</el-button>
      </div>
        ...
        <el-tab-pane :label="obj.project_name" :name="obj.ID" v-for="obj in project_list">
          <div>
            <el-descriptions title="" border :column="4">
              ...
              <el-descriptions-item label="创建时间">{{ formatDate(obj.created_at) }}</el-descriptions-item>
              <el-descriptions-item label="更新时间">{{ formatDate(obj.updated_at) }}</el-descriptions-item>
              <el-descriptions-item label="模板内容">{{ win.project.sms_template.template_content }}</el-descriptions-item>

            </el-descriptions>

            <el-table ref="multipleTable" :data="rows" style="width: 100%;margin-top: 20px;" tooltip-effect="dark"
              row-key="ID" @selection-change="handleSelectionChange">
              <el-table-column type="selection" width="55" align="center" />
              <el-table-column align="center" label="添加日期" width="170">
                <template #default="scope">
                  <span>{{ formatDate(scope.row.created_at) }}</span>
                </template>
              </el-table-column>
              <el-table-column align="center" label="手机号" prop="phone" width="120" />
              <el-table-column align="center" :label="'参数' + num" :prop="`param${num}`" width="120"
                v-for="num in obj.param_num" />

              <el-table-column align="center" label="按钮组" min-width="160" fixed="right">
                <template #default="scope">
                  ...
                  <el-popover v-model="scope.row.visible" placement="top" width="160">
                    ...
                    <div style="text-align: right; margin-top: 8px;">
                      ...
                      <el-button type="primary" @click="DelRow(scope.row.ID)">确定</el-button>
                    </div>
                    <template #reference>
            ...
            <div class="gva-pagination" style="justify-content: space-between;">
              <div class="gva-btn-list" style="margin-top: 12px;">
                <el-button type="primary" icon="plus" size="default" @click="dialogFormVisible2 = true">新增行</el-button>
                <el-button type="primary" icon="delete" size="default" @click="DelRows">删除</el-button>

                <el-button type="primary" icon="printer" size="default" @click="exportExcel">导出excel</el-button>
                <el-button type="warning" icon="message" size="default" @click="send">批量发送信息</el-button>
              </div>

              <div>
                <el-pagination :current-page="pageSetting.page" :page-size="pageSetting.pageSize"
                  :page-sizes="[10, 30, 50, 100]" :total="pageSetting.total"
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="新建项目">
      <el-form :inline="false" :model="form" label-position="top">
        <el-form-item label="项目名">
          <el-input v-model="form.project_name" autocomplete="off" />
        </el-form-item>
        <el-form-item label="模板ID">
          <el-popover :visible="visibleMbID" placement="bottom" :width="400" trigger="click">
            <template #reference>
              <el-input slot="reference" v-model="form.template_id" @click="visibleMbID = true"
                @blur="visibleMbID = false" />
            </template>
            <el-table :data="tableData2" ref="singleTable" highlight-current-row style="width: 100%"
    ...
    <el-dialog v-model="dialogFormVisible2" title="新增行">
      ...
      <el-row>
        <el-input v-model="rowCommit.tpl_phones" :autosize="{ minRows: 5, maxRows: 15 }" type="textarea"
          placeholder="手机号列表" />
      </el-row>
...
<script setup>
import {
  AddSmsProject, GetAllSmsProject, DelSmsProject,
  SmsProjectRows, AddSmsProjectRow, DelSmsProjectRow,
  DelSmsProjectRows, SendSmsByRows, ExportExcelSmsRows
} from '@/api/txyun/sms.js'
